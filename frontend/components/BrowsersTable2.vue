<template>
  <table class="BrowsersTable" cellspacing="0" cellpadding="0" >
    <thead>
      <tr>
        <td v-for="field in availableFields">
          {{ field.name }}
        </td>
      </tr>
    </thead>
...
export default {
  ...
  data: () => ({
    openRows: [],
    visibleFields: [{ name: 'name' }, { name: 'version' }]
  }),
  computed: {
    availableFields() {
      return [{name: ''}, ...Object.keys(this.browsersData[0]).map( field => ({
        name: field,
      }))]
    },
    uniqueBrowserNames() {
  ...
  methods: {
    ...
    closeRow(event, index) {
      console.log('openRow', index);
    },
    resetDateFilter() {
